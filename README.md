...
사이드바(파라미터)와 뷰포트(장면) 한 화면입니다. 스크롤은 사이드바에서만 합니다.

| 영역 | 하는 일 |
|---|---|
| 헤더 4단계 | 씨딩 → 이중 노출 → 상호상관 → 속도장. 누르면 그 단계의 화면이 되고 오른쪽에 한 줄 설명이 붙습니다 |
| 사이드바 · 유동 / 촬영 조건 | 유동 종류 10가지, 입자 변위, 창당 입자 수, 입자상 지름, 노이즈, 면외 손실 |
| 사이드바 · 계산 조건 | 조사창 16/32/64 px, 중첩률 0~75 %, 패스 1~3, 부화소 피팅 3종, 이상치 문턱값 |
| 사이드바 · 보기 | 배경 7종, 겹쳐 그리기(벡터·참값·오차×10·격자), 화살표 길이 |
| 뷰포트 왼쪽 위 · 측정 요약 | RMS 오차와 부화소 정밀도, 벡터 수·유효율·평균 피크·편향·패스 구성·계산 시간 |
| 뷰포트 오른쪽 위 · 상관 현미경 | 클릭한 조사창의 창 A, 창 B, 상관면 R(Δx,Δy)를 입체/평면으로. 측정값·참값·오차·SNR까지 |
| 뷰포트 왼쪽 아래 · 범례 | 색 띠와 양 끝값, 화살표 기준자, 이 장이 무엇인지 한 줄 |
| 뷰포트 오른쪽 아래 · 경고 | 물리 기준으로 걸리는 것이 있을 때만, **한 번에 하나만** 뜹니다 |

모든 수치 파라미터는 슬라이더 + 숫자칸 + 유도값 힌트 세 짝입니다. 힌트는 그 값을
바꾸면 무엇이 따라 변하는지 즉시 보여 줍니다 — 예를 들어 조사창을 32 px 로 두면
"벡터 간격 16 px · 측정 한계 ±8.0 px", 변위를 9 px 로 올리면 "창 32 px 의 1/4 =
8.0 px 를 1.0 px 넘습니다" 로 바뀌고 오른쪽 아래에 고칠 방법이 함께 뜹니다.
...
「디자인 지침 — THINK Lab 해석도구 공통」을 따릅니다. 토큰·컴포넌트·레이아웃은
지침 그대로이고, 이 도구의 사정으로 벗어난 세 가지(헤더 2분할을 4단계로, 입자
영상 반전, 부호가 있는 장의 발산형 색과 범위 하한)는 이유와 함께 `DESIGN.md` 에
적어 두었습니다. 웹폰트를 포함해 외부 요청이 하나도 없어서 인터넷 없이도 열립니다.

## 파일

```
index.html          문서 구조 (문구는 전부 data-i18n 키)
DESIGN.md           디자인 지침 적용 내역과 벗어난 것
icon.svg            도구 아이콘 (THINKLAB 홈페이지 도구 목록이 이 주소를 읽습니다)
css/piv.css         공통 디자인 토큰 · 레이아웃
...
js/colormap.js      색 램프 (viridis · 발산형)
js/render.js        캔버스 그리기 (벡터, 장, 상관면 입체)
js/i18n.js          한국어 / 영어 문구 (값이 들어가는 힌트·경고 포함)
js/app.js           상태와 배선
